...
const DataModal = ({ subTitle, handleOpenModal, handleOpenEditModal, open, val }) => {
  const [plan, setPlan] = useState(val ? val?.plan : '');
  const [amount, setAmount] = useState(val ? val?.amount : null);
  const [duration, setDuration] = useState(val ? val?.duration : '');
  ...
  const createPlan = (data) => {
    axios
      .post('http://localhost:4000/api/subscription/data/addplan', data)
      .then(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!plan || !amount || !duration || !network) {
    ...
    <div>
      ...
      >
        <Box sx={style}>
          ...
          <br />
          <br />
          <TextField id="duration" label="Duration" value={duration} onChange={(e) => setDuration(e.target.value)} />
          <br />
          <br />
